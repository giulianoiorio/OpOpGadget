...
class Analysis:
    def __init__(self, particles=None, safe=True, auto_centre=False, iter=False, **kwargs):
        ...
        if 'filename' in kwargs:
            self.p = load_snap(kwargs['filename'])
        elif isinstance(particles, Particles):
            if safe == True:
                self.p = copy.deepcopy(particles)
            else:
                self.p = particles
        else:
            raise IOError('Incorrect particles or filename')
    ...
    def center(self, mq, iter=False, single=True, type=None, fac=0.975, maxiter=500, po=(0, 0, 0), vo=(0, 0, 0), ):
        ...
        if single == False:
            ...
            if iter:
                com, vcom = self.comit(fac=fac, limit_mass=mq, maxiter=maxiter, type=type)
            else:
            ...
            for i in range(6):
                ...
                if co != 0:
                    ...
                    if iter:
                        com, vcom = self.comit(fac=fac, limit_mass=mq, maxiter=maxiter, type=[i])
                    else:
    ...
    def _make_array(self, original_array, type):
        ...
        if len(original_shape) == 1:
            ...
            raise ValueError('Original array dimension need to have 1 or 2 axis')
    ...
    def _set_tensor(self, value_array, weight_array, eig=True):
        ...
        if eig == True:
            return mat2, eigh(mat2, eigvals_only=True)
        else:
            return mat2
